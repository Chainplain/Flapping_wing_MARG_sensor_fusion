...
class Ave_Com_MARG_Filter():
    ...
    def march_forward(self, omega_m_in_mat, a_m_in_mat, m_m_in_mat, f_F):
        ...
        a_m_est_raw = accel_ave #+ 0.2 * a_v_est
        ...
        # delta_q_g_est = (0.5 * Omega(omega_ave - self.b_g_est) )*self.q_est * self.step
        ...
        mu_k = np.linalg.norm(delta_q_g_est) *50
        # print('mu_k: ',mu_k)
        ...
        b_g_norm = np.linalg.norm(self.b_g_est)
        if (b_g_norm > 1):
            self.b_g_est = self.b_g_est / b_g_norm * 1
...
class OSC_LEARNER():
    ...
    def march_forward(self, gyro_meas, acc_meas, mag_meas, f_F):
        ...
        T_F = int(math.floor( 1 / f_F / self.step + self. delta_T_est ))
        if T_F < 0 :
            T_F = 0
        ...
        x_F_est = self.est_Data_list[T_F] + self.os_learning_rate * (x_F - self.est_Data_list[T_F])
        for i in range(1, self.Length_max * 2):
        ...
        q_F_ave_dual = np.mat([q_F_ave_normed[0,0], -q_F_ave_normed[1,0], -q_F_ave_normed[2,0], -q_F_ave_normed[3,0]]).T
        ...
        self.q_F_est = self.q_F_est + (0.5 * Omega(omega_add_tog) - 0.125 * Omega(omega_add_tog) * Omega(omega_add_tog)) * \
